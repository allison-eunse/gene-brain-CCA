#!/usr/bin/env python3
"""
Coupling Benchmark v2: CCA/SCCA grid with optional extra covariates (e.g., eTIV).

Changes vs v1:
- Adds --cov-extra (1D or 2D array) to include additional nuisance covariates
  in leakage-safe residualization (train-only fit, apply to val/holdout).
"""
...
def _build_cov(age: np.ndarray, sex: np.ndarray, extra: np.ndarray | None = None) -> np.ndarray:
    """(N,) age/sex + optional extra covariates -> (N, 3+K) cov matrix with intercept."""
    cols = [
...
def _residualize_train_val(
    ...
    """
    Fit linear residualization on TRAIN only (intercept+age+sex+extra),
...
def _run_config_cached(
    *,
    ...
    for fold_idx, gf in enumerate(gene_cache["folds"]):
        ...
        r_train, r_val = _fit_and_score(
            method=method, Xg_tr=Xg_tr, Xb_tr=Xb_tr, Xg_te=Xg_va, Xb_te=Xb_va, k=k, c1=c1, c2=c2
        )
        fold_results.append(
            {
                "fold": fold_idx,
    ...
    k = min(n_components, Xg_tr.shape[1], Xb_tr.shape[1])
    r_train_final, r_holdout = _fit_and_score(
        method=method, Xg_tr=Xg_tr, Xb_tr=Xb_tr, Xg_te=Xg_ho, Xb_te=Xb_ho, k=k, c1=c1, c2=c2
    )
    holdout_stats = {
...
def main():
    ...
    ap.add_argument("--cov-extra", default=None, help="Optional extra covariates (.npy), shape (N,) or (N,C)")
    ...
    if shm_free_gb > gene_size_gb * 1.2:
        ...
        use_shm = True
        print("  Gene matrix copied to RAM.", flush=True)
    else:
    ...
    cov_extra = _as_2d(cov_extra)

    print(f"  Gene embeddings: {Xg_raw.shape}", flush=True)
    print(f"  Subjects: {len(labels)}", flush=True)

    if cov_extra is not None and cov_extra.shape[0] != len(labels):
        raise SystemExit(f"[ERROR] cov-extra row mismatch: {cov_extra.shape} vs N={len(labels)}")
    ...
    if use_shm and shm_gene_path.exists():
        print("  Cleaning up /dev/shm...", flush=True)
